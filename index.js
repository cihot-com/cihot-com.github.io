...
setTimeout(function () {
	...
	s.on('connect', () => {
		...
		if (
			...
			isNaN(parseInt(time))
			||
			((7 * 24 * 60 * 60 * 1000) < (Date.now() - parseInt(time)))
		) {
...
document.querySelector('#find_other_user_source').addEventListener('keydown', (event) => {
	let projectName = location.search.replace(/[^A-Za-z0-9]/g, '')
	let { keyCode, target } = event
	if (keyCode === 13 && /@gmail.com$/.test(document.documentElement.getAttribute('gmail'))) {
		event.preventDefault()
		...
		if (text) {
			let div = document.querySelector('#find-panel')
			let isSource = target.parentElement.elements.namedItem('find_column_name').value === 'source'
...
function saveDatas() {
	...
	let useAutoReplaceTarget = document.querySelector('#useAutoReplaceTarget')
		.checked
...
function upload_dicts() {
	...
	if (isNaN(lastAutoUploadTime) || (Date.now() - lastAutoUploadTime) > 7.2e6) {
		let i = 0
		tm.iterate((d, k) => {
			if (k !== '_test' && d && (typeof d === 'object') && (Array.isArray(d.dictArray))) {
				setTimeout(() => {
...

	{
		...
		$(document).on('focus', '#works .target', async function (e) {
			...
			if (sessionStorage.getItem('sl') && sessionStorage.getItem('tl')) {
				let s = $(e.target).parent('tr').find('.source').text().trim()
				translate(s, sessionStorage.getItem('sl'), sessionStorage.getItem('tl'), (d) => {
			...
			if (e.keyCode === 19 && !e.repeat) {
				...
				calc = function () {
					...
					result.forEach(text => {
						...
						el.textContent = text
						el.style.backgroundColor = '#' + Math.random().toString(16).substr(2, 6) + '33'
					...
					function makeRegExp(s) {
						//优化：去掉源空格
						let regEpxSource = s.split('').filter(e => !/\s/.test(e)).map((e, i) => {
							if (i === 0) {
								return e
							} else {
								// 重点！！不用加
		...
		$('#downloadWorksExcel').on('click', function (e) {
			...
			if (e.ctrlKey) {
				let rs = []
				document.querySelectorAll('#works tbody[dataname]').forEach((tbody, index) => {
		...
		function tagCheck(s, t) {
			...
			let tagRE = /[^\sA-Za-z0-9\u4e00-\u9fa5\u3131-\u314e\u314f-\u3163\uac00-\ud7a3\u3040-\u30ff\u31f0-\u31ff]/g
		...
		$('#restoreButton').click(() => {
			...
			bd.getItem(formatName(location.search) + 'backuptime', (j, v) => {
				if (j) {
					...
					if (
						confirm('Last backup time is:  ' + new Date(v).toLocaleString())
					) {
						restore()
					}
				}
...
function splitLongSource(str) {
	...
	ss.split(/.+?(\!\?|\?\!|\?|\!|\.)+(\s*)/)
	//ss.split(/(\s*),(\s*)/)
	ss.split(/。|；|：|！/, undefined, ({s, beforeStr, afterStr, matchedAllStr, matchedIndex})=>{
...
function gSearch(s, t) {
	...
	if (isUseNet) {
		...
		//清空上一次结果
		document.querySelectorAll('.search-engine-results .source, .search-engine-results .target').forEach(el => el.textContent = '')
	}
	if (isUseNet2) {
...
function onmousedownHandleByNo(e) {
	...
	if (e.which === 1) {
		if (e.shiftKey) {
			...
			let isStartNoSelected = lastSelectedNoElement.classList.contains(
				'selected'
			) // 开始No的状态
			let count = 0
			do {
...

{
	...
	$('#projectAnchors').on('contextmenu', (e) => {
		...
		if (target.nodeName === 'A') {
			...
			if (!confirm(`경고! 프로젝트(${name}) 삭제는 복구 불가하니, 미리 백업하시기 바랍니다. 지금 삭제하시겠습니까?`)) return false
...
window.addEventListener('message', (e) => {
	...
	function applyWorks(source, begin, end, target) {
		let trs = Array.from(document.querySelectorAll('#works tr:not(.hide):not(.hide2)')).filter(
			(tr) => {
				return (
					!tr.classList.contains('hide') &&
					!tr.classList.contains('hide2') &&
...
document.querySelector('#showAllButton')
	.addEventListener('click', showAll)

document.querySelector('#clearTargetButton')
...
requestAnimationFrame(() => {
	...
	new MutationObserver((mutations, observer) => {
		let searchLength = Number(document.querySelector('#doneAutoReplaceLength').value)
		...
		if (sIns.length === 1 && sDel.length === 1) {
			...
			if (ins.length > searchLength || del.length > searchLength) return console.log('超出搜索长度')
...
function printTips() {
	let arr = Array.from(document.querySelector('#tips').querySelectorAll('tr'), (tr) => { return [tr.querySelector('.source').textContent, tr.querySelector('.target').textContent] })
	console.log(arr)
...
document.querySelector('#removeDictForDoc').addEventListener('click', () => {
	...
	Array.from(document.querySelectorAll('#works tr:not(.hide):not(.hide2)')).some((tr, i) => {
		let s = tr.querySelector('.source')
		let t = tr.querySelector('.target')
		let sx = s.textContent
		let tx = t.textContent
		dict.array.forEach(([dsx, dtx], di) => dsx === sx && dtx === tx ? delete_indexs.add(di) : false)
	})
...
document.querySelectorAll('#toolleft>*>h3:first-child, #toolright>*>h3:first-child').forEach(el => {
...
function translate(sourceText, sourceLanguage, targetLanguage, ack) {
	//tm4ext插件
	return fetch('chrome-extension://egdgnkojcllghibmckednllfpgjhbgbl/manifest.json')
		.then(d => {
			//安装了插件
			if (d.ok) {
				//has
			...
			if (presLength) {
				...
				if (plus) {
					if (isNaN(quickInsertTranslateIndex)) {
						...
						quickInsertTranslateIndex = loopIndex(++quickInsertTranslateIndex, presLength)
					}
				} else {
					if (isNaN(quickInsertTranslateIndex)) {
