...
index: - ${i} -`)) {
	...
	*/
	...
	function gSearch(s, t) {
		...
		if (!gSearch.google || gSearch.google.closed) gSearch.google = wopen(``, 'google', { left: 0, top: 0, width: 300 });
		gSearch.google.location.href = `https://translate.google.cn/#view=home&op=translate&sl=auto&tl=${t}&text=${s}`;

		if (!gSearch.papago || gSearch.papago.closed) gSearch.papago = wopen(``, 'papago', { left: 0, top: 400 });
		if (t === 'jp' || t === 'ja') {
			t = 'ja';
		}
		gSearch.papago.location.href = `https://papago.naver.com/?sk=auto&tk=${t}&st=${s}`;

		if (!gSearch.googleAll || gSearch.googleAll.closed) gSearch.googleAll = wopen(``, 'googleAll', { left: 0, top: 800 });
...
function match100(range) {
	...
	match100fns = []
	match100chain(range.toArray().map(tr => clac.bind(this, tr)))
...
function smartMatch(source, sourceTargetArray) {log(sourceTargetArray)
	var ret = '';
	var o = strDiff(source, sourceTargetArray[0]);
	var d1 = o.diff1, d2 = o.diff2, len1 = d1.length, len2 = d2.length, d1Value, d2Value;
	var regexp = /^[\x01-\xff]+$/;
