void function () {
	...
	class History extends Array {
		constructor(opt) {
			...
			if (opt) {
				if (opt.length) this.options.length = Math.min(opt.length, History.maxLength) || 30;
				if (opt.keyup) this.options.keyup = true
				if (!opt.keydown) this.options.keydown = false
			}
		}
