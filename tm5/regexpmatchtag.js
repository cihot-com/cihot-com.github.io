...
let s = `① 길드 보상{0}.
획득 점수가 [A3DB57FF]1,000점[-] 미만 시 지급되지 않습니다.
+9,999,999
555, 555, 55-9.8%
.837890%  4000km
/v0
...
let r = new RegExp(rs.map((e) => e.source).join('|'), 'gui')
...
!function () {
	...
	\\p{Uppercase_Letter}`
	ps.split('\n').forEach(e => {
		let re = new RegExp(e + '+', 'gu')
		log(e, s.match(re))
	})


	let ext = `\\p{Script_Extensions=Adlam}
