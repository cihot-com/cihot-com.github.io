...
// md5 module
(function(a){if(typeof module!=="undefined"){if(module.parent){exports=a("nodejs")}else{global.md5=a("nodejs")}}else{if(typeof window!=="undefined"){window.md5=a("brouwser")}}})(function(c,f){var e=function(){};e.chunk=2097152;if(c==="nodejs"){var a=require("fs");e.file=function b(q,h){var j=a.openSync(q,"r"),r=a.statSync(q).size,l=e.chunk,o=Math.ceil(r/l),m=0,n,g,p,k;n=new sparkmd5.ArrayBuffer();g=Buffer.alloc(l);while(m<o){p=l*m;k=a.readSync(j,g,0,l,p);if(k<l){g=g.slice(0,k)}n.append(g);m++;if(h){h(m,o,g)}}a.closeSync(j);return n.end()};e.string=function d(o,g){var p=o.length,k=e.chunk,h,n=Math.ceil(p/k),l=0,m=new sparkmd5.ArrayBuffer(),j;while(l<n){j=o.substr(k*l,k);h=Buffer.from(j,"utf8");m.append(h);l++;if(g){g(l,n,j)}}return m.end()}}else{if(c==="brouwser"){e.file=function(h,g,p){var o=File.prototype.slice||File.prototype.mozSlice||File.prototype.webkitSlice,i=e.chunk,k=Math.ceil(h.size/i),n=0,l=new SparkMD5.ArrayBuffer(),j=new FileReader();if(p===f){p=g;g=f}window.fr=j;j.onload=function(s){l.append(s.target.result);n++;if(n<k){m();if(g){g(n,k,s.target.result)}}else{var r=l.end(),q=new Event("end");q.data=s.target.md5=r;s.target.dispatchEvent(q);if(g){g(n,k,r)}if(p){p(r)}}};function m(){var r=n*i,q=((r+i)>=h.size)?h.size:r+i;j.readAsArrayBuffer(o.call(h,r,q))}m();return j};e.string=function d(o,g){var p=o.length,k=e.chunk,h,n=Math.ceil(p/k),l=0,m=new sparkmd5.ArrayBuffer(),j;while(l<n){j=o.substr(k*l,k);h=new Blob([j]);m.append(h);l++;if(g){g(l,n,j)}}return m.end()};return window.md5=e}}return e});